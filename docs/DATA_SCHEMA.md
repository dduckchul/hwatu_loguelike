...
같은 정의의 카드를 여러 장 가질 수 있지만 각 장은 별도 `CardInstance`다. 강화는 원본 `Normal` 카드를 같은 월의 특수 카드 정의를 가진 새 인스턴스로 교체하며 카드 수와 위치를 유지한다.
...
가격과 구매 여부는 `CardData`에 저장하지 않는다. 강화와 제거의 방문별 사용 상태, 비용 데이터와 대상 목록은 아직 구현하지 않았다.
