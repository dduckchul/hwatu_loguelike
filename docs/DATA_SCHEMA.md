...
현재 카드 보상 기능은 상점 상단의 카드 구매 목록으로 확장할 예정이다. 상점 방문마다 구매 횟수를 런타임 상태로 관리하며 첫 구매 비용은 0이다. 두 번째 이후 카드 가격은 공식이 확정된 뒤 일반 C# 가격 규칙으로 계산하고, `CardData`에는 런타임 가격이나 구매 상태를 저장하지 않는다.
...
- 두 번째 이후 카드 구매 가격 공식에 필요한 밸런스 데이터
